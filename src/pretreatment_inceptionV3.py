...
model_test.load_weights('./models/pretreatment_models/inception_v3_0802_edit.h5')
...
build_csv(filenames, test_np, './results_tmp/pretreatment_0.csv')
...
for i in tqdm(range(n_test)):
    ...
    X_test[i] = cv2.resize(img, (width, height), interpolation=cv2.INTER_AREA )
test_np = model_test.predict(X_test, batch_size=100, verbose=1)
build_csv(filenames, test_np, './results_tmp/pretreatment_1.csv')
...
for i in tqdm(range(n_test)):
    ...
    X_test[i] = cv2.resize(img, (width, height), interpolation=cv2.INTER_AREA )
test_np = model_test.predict(X_test, batch_size=100, verbose=1)
build_csv(filenames, test_np, './results_tmp/pretreatment_2.csv')
